...
async function get_old_playlists(fn, matches) {

    console.log('Example of how the getItunesPlaylists function works!');
    console.log('Begin get_old_playlists(' + fn + ',' + matches + ')');
    ...
    return new Promise(function (resolve, reject) {
        ...
        stream.on('data', function (res) {
            ...
            for( var m = 0; m < matches.length; m++) {
                if (playlist['Name'].startsWith(matches[m])) {
...
async function zip(oldfn, newfn, playlist_match, output_fn) {
    ...
    if (dat.playlists.length > 0) {
        ...
        for (var i = 0; i < dat.playlists.length; i++) {
            ...
            buffer += "\t\t<dict>\n";
            buffer += "\t\t\t<key>Name</key><string>" + escapeXml(dat.playlists[i].Name) + "(fixed)</string>\n";
            ...
            for (var j = 0; j < dat.playlists[i].tracks.length; j++) {
                ...
                if( !nt ) {
                    console.log("no match for track " + (j+1) + " in " + dat.playlists[i].Name);
                    console.log(t + " : " + l.Name + ":" + l.Location);
                } else {
                    buffer += "\t\t\t\t<dict>\n";
...
if (args.length < 4) {
    ...
    const playlist_match = args.slice(3);
